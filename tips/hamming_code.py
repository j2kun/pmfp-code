# Nb., bitstring is not all that performant, but python's struct.pack/unpack
...
def decode(bits: BitStream) -> BitStream:
    ...
    for i in range(len(bits) // 7):
        ...
        # Requires Python 3.10 for bit_count
        syndrome = (
            4 * ((block & 108).bit_count() & 1)
            + 2 * ((block & 90).bit_count() & 1)
            + ((block & 57).bit_count() & 1)
        )
        block ^= syndrome_table[syndrome]
        decoded = ((block & 64) + (block & 32) + (block & 16) + (block & 8)) >> 3
