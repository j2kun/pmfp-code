...
def move_vertex(
    ...
    print(f"Moving {vertex_id=} from \n\t{current_community} to\n\t{target_community}")
...
def compute_objective_change(
    ...
    for other_id in current_community.vertices | target_community.vertices:
        ...
        after_distance = 0 if other_community == current_community.id else 1
        change += rescaled_weight * ((1 - after_distance) - (1 - before_distance))

    return change


def louvain(graph: igraph.Graph, resolution: float) -> set[Community]:
    ...
    while True:
        ...
        for i in random.sample(range(vertex_count), vertex_count):
            ...
            if largest_change > 1e-08:
                print(f"Objective before move={_compute_objective(graph, resolution)}")
                move_vertex(graph, i, current_community, best_community)
                print(f"Objective after move={_compute_objective(graph, resolution)}")
                had_change = True
