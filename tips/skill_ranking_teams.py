...
def truncated_twosided_gaussian_w(perf_diff, draw_margin):
    ...
    v_value = truncated_twosided_gaussian_v(perf_diff, -draw_margin, draw_margin)
    t1 = (draw_margin - abs_diff) * STANDARD_NORMAL.pdf(draw_margin - abs_diff)
    t2 = (-draw_margin - abs_diff) * STANDARD_NORMAL.pdf(-draw_margin - abs_diff)
    return v_value**2 + (t1 - t2) / normalization


def update_one_team(team1: Team, team2: Team, outcome: int) -> Dict[Player, Rating]:
